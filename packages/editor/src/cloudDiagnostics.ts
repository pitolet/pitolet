...
type Context = { workspaceId: string | null; documentId: string | null };
let context: Context = { workspaceId: null, documentId: null };
let enabled = false;
const recent: string[] = [];
...
export function setCloudProblemContext(next: Partial<Context>): void {
  context = { ...context, ...next };
}

export function setCloudDiagnosticsEnabled(value: boolean): void {
  enabled = value;
}

export function recentCloudErrors(): string[] {
  return [...recent];
}

export function reportCloudEditorProblem(
  error: unknown,
  component?: string,
  fallback = 'Browser error',
): void {
  if (!serverBase || isShareSession || !enabled) return;
...
export function installCloudProblemReporter(): () => void {
  if (!serverBase || isShareSession) return () => {};
  const onError = (event: ErrorEvent) =>
    reportCloudEditorProblem(event.error ?? event.message, undefined, 'Browser error');
  const onRejection = (event: PromiseRejectionEvent) =>
    reportCloudEditorProblem(event.reason, undefined, 'Unhandled promise rejection');
...
export async function initializeCloudDiagnostics(): Promise<() => void> {
  if (!serverBase || isShareSession) return () => {};
  try {
    ...
    if (response.ok) {
      ...
      if (session.kind === 'user' && session.workspace?.id) {
        setCloudProblemContext({ workspaceId: session.workspace.id });
        setCloudDiagnosticsEnabled(true);
      }
    }
  } catch {
    // Diagnostics are optional. A failed context check must not delay the editor.
  }
  return installCloudProblemReporter();
}
